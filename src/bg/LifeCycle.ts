...
var LifeCycle = (() => {
  ...
  var LifeBoat = {
    ...
    async createAndStore() {
      ...
      if (!tab) {
        // no suitable existing tab, let's open a new one
        if (!UA.isMozilla) {
      ...
      let tabId = tab.id
      let { url } = tab
      let { cypherText, key, iv } = await encrypt(
        JSON.stringify({
      ...
      try {
        await new Promise((resolve, reject) => {
          let l = async (tabId, changeInfo) => {
            ...
            try {
              attr = await Messages.send(
                "store",
                { url, data: toBase64(new Uint8Array(cypherText)) },
                { tabId, frameId: 0 }
              )
              resolve()
        ...
        await Storage.set("local", {
          updateInfo: { key, iv: toBase64(iv), tabId, url, attr },
        })
        tabId = -1
    ...
    async retrieveAndDestroy() {
      ...
      try {
        ...
        for (let attempts = 3; attempts-- > 0; ) {
          try {
            cypherText = await Messages.send(
              "retrieve",
              { url, attr },
              { tabId, frameId: 0 }
            )
            ...
            if (Messages.isMissingEndpoint(e)) {
              debug(
                "Cannot retrieve survival tab data, maybe content script not loaded yet. Retrying..."
              )
        ...
        let { policy, allSeen, unrestrictedTabs } = JSON.parse(
        ...
        await ns.initializing
        await Promise.all(
          Object.entries(allSeen).map(async ([tabId, seen]) => {
            try {
  ...
  return {
    async onInstalled(details) {
      ...
      if (!UA.isMozilla) {
        ...
        let contentScripts = browser.runtime
          .getManifest()
      ...
      if (Ver.is(previousVersion, "<=", "11.0.10")) {
        log(
          `Upgrading from 11.0.10 or below (${previousVersion}): configure the "ping" capability.`
        )
        await ns.initializing

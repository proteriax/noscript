...
XSS.InjectionChecker = (async () => {
  ...
  const IC_WINDOW_OPENER_PATTERN =
    fuzzify("alert|confirm|prompt|open(?:URL)?|print|show") + "\\w*" + fuzzify("Dialog")
  const IC_EVAL_PATTERN =
    "\\b(?:" +
    fuzzify(
      "eval|import|set(?:Timeout|Interval)|(?:f|F)unction|Script|toString|Worker|document|constructor|generateCRMFRequest|jQuery|fetch|write(?:ln)?|__(?:define(?:S|G)etter|noSuchMethod)__|definePropert(?:y|ies)"
    ) +
    "|\\$|" +
    IC_WINDOW_OPENER_PATTERN +
    ")\\b"
  const IC_EVENT_PATTERN =
  ...
  InjectionChecker.prototype = {
    reset() {
      this.isPost = this.base64 = this.nameAssignment = false
    ...
    maybeMavo(s) {
      return (
        /\[[^]*\([^]*\)[^]*\]/.test(s) &&
        /\b(?:and|or|mod|\$url\b)/.test(s) &&
        this.maybeJS(
          s.replace(/\b(?:and|or|mod|[[\]])/g, ",").replace(/\$url\b/g, "location"),
          true
        )
      )
    },
    ...
    reduceQuotes: function (s) {
      ...
      if (
        /\/\*/.test(s) || // C-style comments, would make everything really tricky
        /\w\s*(\/\/[\s\S]*)?\[[\s\S]*\w[\s\S]*\]/.test(s)
      ) {
        // property accessors, risky
        return s
      }
      ...
      for (;;) {
        ...
        while (start > -1 && end - start > 1) {
          ...
          if (await this.timing.pause()) {
            this.log(
              `JSON reduction iterations ${iterations++}, elapsed ${
                this.timing.elapsed
              }, expr ${expr}`
            )
          }
          ...
          try {
            if (!toStringRx.test(JSON.parse(expr).toString)) continue
          ...
          if (
            !/[(=.]|[^:\s]\s*\[|:\s*(?:location|document|set(?:Timeout|Interval)|eval|open|show\w*Dialog|alert|confirm|prompt)\b|(?:\]|set)\s*:/.test(
              qred
            ) &&
    ...
    maybeJS(expr, mavoChecked = false) {
      ...
      if (
        /`[\s\S]*`/.test(expr) || // ES6 templates, extremely insidious!!!
        this._evalAliasingRx.test(expr) ||
        this._riskyOperatorsRx.test(expr) // this must be checked before removing dots...
      )
        return true

      expr = expr // dotted URL components can lead to false positives, let's remove them
        .replace(this._removeDotsRx, this._removeDots)
      ...
      if (this._assignmentRx.test(expr) && !this._badRightHandRx.test(expr))
        // commonest case, single assignment or simple chained assignments, no break
        return (
          this._singleAssignmentRx.test(expr) ||
          (this._riskyAssignmentRx.test(expr) && this._nameRx.test(expr))
        )

      return (
        this._riskyParensRx.test(expr) ||
    ...
    get invalidCharsRx() {
      ...
      return (this.invalidCharsRx = new RegExp(
        "^[^\"'`/<>]*[" + this._createInvalidRanges() + "]"
      ))
    },
    ...
    async checkJSBreak(s) {
      ...
      s =
        ";" +
        this.reduceXML(
          await this.reduceJSON(
            this.collapseChars(
              s.replace(/\%\d+[a-z\(]\w*/gi, "§").replace(/[\x01-\x09\x0b-\x20]+/g, " ")
            )
          )
        ).replace(/[\r\n\u2028\u2029]+/g, "\n")

      if (s.indexOf("*/") > 0 && /\*\/[\s\S]+\/\*/.test(s)) {
        // possible scrambled multi-point with comment balancing
        s += ";" + s.match(/\*\/[\s\S]+/)
      }
      ...
      const invalidCharsRx = /[\u007f-\uffff]/.test(s) && this.invalidCharsRx,
      ...
      for (let dangerPos = 0, m; (m = injectionFinderRx.exec(s)); ) {
        ...
        for (let len = expr.length, moved = false, hunt = !!expr, lastExpr = ""; hunt; ) {
          if (await this.timing.pause()) {
            this.log(
              `Elapsed ${this.timing.elapsed}ms, taken a ${this.timing.pauseTime}ms nap.`
            )
          }
          ...
          if (this.maybeJS(this.reduceQuotes(script))) {
            ...
            if (this.syntax.lastError) {
              ...
              if (
                /invalid .*\bflag\b|missing ; before statement|invalid label|illegal character|identifier starts immediately/.test(
                  errmsg
                )
              ) {
                if (
                  errmsg.indexOf("illegal character") === -1 &&
                  /#\d*\s*$/.test(script)
                ) {
                  // sharp vars exceptional behavior
                  if (!quote) break
    ...
    HTMLChecker: new RegExp(
      "<[^\\w<>]*(?:[^<>\"'\\s]*:)?[^\\w<>]*(?:" + // take in account quirks and namespaces
        fuzzify(
          "script|form|style|svg|marquee|(?:link|object|embed|applet|param|i?frame|base|body|meta|ima?ge?|video|audio|bindings|set|isindex|animate|template"
        ) +
        ")[^>\\w])|['\"\\s\\0/](?:style|innerhtml|data-bind|(?:data-)?mv-(?:\\w+[\\w-]*)|" +
        IC_EVENT_PATTERN +
        ")[\\s\\0]*=|<%[^]+[=(][^]+%>",
      "i"
    ),
    ...
    async checkHTML(s) {
      ...
      if (links) {
        for (let l of links) {
          ...
          if (
            /^(?:javascript|data):|\[[^]+\]/i.test(l) ||
            (/[<'"(]/.test(unescape(l)) && (await this.checkUrl(l)))
          )
            return true
        }
      }
    ...
    async checkNoscript(s) {
      ...
      return (
        (s.indexOf("\x1b(J") !== -1 &&
          (await this.checkNoscript(s.replace(/\x1b\(J/g, "")))) || // ignored in iso-2022-jp
        (s.indexOf("\x7e\x0a") !== -1 &&
          (await this.checkNoscript(s.replace(/\x7e\x0a/g, "")))) || // ignored in hz-gb-2312
    ...
    async checkBase64(url) {
      ...
      while (parts.length) {
        if (
          (await this.checkBase64Frag(parts.join("/"))) ||
      ...
      if (this.base64tested.indexOf(f) < 0) {
        ...
        try {
          ...
          if (
            ...
            ((await this.checkHTML(s)) || (await this.checkAttributes(s)))
    ...
    async checkBase64FragEx(f) {
      return (
        (await this.checkBase64Frag(Base64.purify(f))) ||
        (await this.checkBase64Frag(Base64.purify(Base64.alt(f))))
      )
    },

    async checkUrl(url, skipRx = null) {
      if (skipRx) url = url.replace(skipRx, "")
      return await this.checkRecursive(
        url
    ...
    async checkRecursive(s, depth = 3, isPost = false) {
      ...
      if (ASPIdiocy.affects(s)) {
        if (await this.checkRecursive(ASPIdiocy.process(s), depth, isPost)) return true
      } else if (
        ASPIdiocy.hasBadPercents(s) &&
        (await this.checkRecursive(ASPIdiocy.removeBadPercents(s), depth, isPost))
      ) {
        return true
      }
    ...
    async _checkRecursive(s, depth) {
      ...
      if (
        s.indexOf("+") !== -1 &&
        (await this._checkRecursive(this.formUnescape(s), depth))
      )
        return true
      ...
      if (
        (unescaped.indexOf("\x1b(J") !== -1 &&
          (await this._checkRecursive(unescaped.replace(/\x1b\(J/g, ""), depth))) || // ignored in iso-2022-jp
        (unescaped.indexOf("\x7e\x0a") !== -1 &&
          (await this._checkRecursive(unescaped.replace(/\x7e\x0a/g, "")))) // ignored in hz-gb-2312
      ) {
        return true
      }
      ...
      if (s != unescaped && (await this._checkRecursive(s, depth))) return true
      ...
      return url.replace(
        ...
        function (m) {
          ...
          switch (hex.length) {
            ...
            case 6:
              c =
                ((parseInt(hex.substring(0, 2), 16) & 0x3f) << 12) |
                ((parseInt(hex.substring(2, 4), 16) & 0x3f) << 6) |
                (parseInt(hex.substring(4, 6), 16) & 0x3f)
              break
            default:
              c =
                ((parseInt(hex.substring(0, 2), 16) & 0x3f) << 6) |
    ...
    urlUnescape: function (url, brutal) {
      ...
      try {
        ...
        if (url != od) url += " (" + od + ")"
        this.log(
          "Problem decoding " + url + ", maybe not an UTF-8 encoding? " + warn.message
        )
        return od
      }
    },

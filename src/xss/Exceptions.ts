...
XSS.Exceptions = (() => {
  const Exceptions = {
    ...
    async shouldIgnore(xssReq) {
      ...
      if (srcOrigin) {
        // srcUrl-specific exceptions

        if (/^about:(?!blank)/.test(srcOrigin)) return true // any about: URL except about:blank

        if (
          srcOrigin === "https://www.youtube.com" &&
          /^https:\/\/(?:plus\.googleapis|apis\.google)\.com\/[\w/]+\/widget\/render\/comments\?/.test(
            destUrl
          ) &&
        ...
        if (isPost) {
          if (
            srcOrigin === "https://sso.post.ch" &&
            destOrigin === "https://app.swisspost.ch"
          ) {
            return true
          }

          if (
            srcOrigin === "https://twitter.com" &&
            /^https:\/\/.*\.twitter\.com$/.test(destOrigin)
          ) {
            return true
          }
          ...
          if (
            /^https?:\/\/csr\.ebay\.(?:\w{2,3}|co\.uk)\/cse\/start\.jsf$/.test(srcUrl) &&
            /^https?:\/\/msa-lfn\.ebay\.(?:\w{2,3}|co\.uk)\/ws\/eBayISAPI\.dll\?[^<'"%]*$/.test(
              unescapedDest
            ) &&
          ...
          if (
            /^https:\/\/(?:cap\.securecode\.com|www\.securesuite\.net|(?:.*?\.)?firstdata\.(?:l[tv]|com))$/.test(
              srcUrl
            ) &&
          ...
          if (
            /\.verizon\.com$/.test(srcOrigin) &&
            /^https:\/\/signin\.verizon\.com\/sso\/authsso\/forumLogin\.jsp$/.test(
              destUrl
            ) &&
          ...
          if (
            /^https?:\/\/mail\.lycos\.com\/lycos\/mail\/MailCompose\.lycos$/.test(
              srcUrl
            ) &&
          ...
          if (
            /\.livejournal\.com$/.test(srcOrigin) &&
          ...
          if (
            srcOrigin == "https://ssl.rapidshare.com" &&
          ...
          if (
            srcOrigin == "http://wm.letitbit.net" &&
            /^http:\/\/http\.letitbit\.net:81\/cgi-bin\/multi\/upload\.cgi\?/.test(
              destUrl
            ) &&
          ...
          if (
            srcOrigin == "https://www.mymedicare.gov" &&
          ...
          if (
            /^https?:\/\/(?:draft|www)\.blogger\.com\/template-editor\.g\?/.test(
              srcUrl
            ) &&
          ...
          if (
            /^https?:\/\/www\.readability\.com\/articles\/queue$/.test(destUrl) &&
          ...
          if (
            /^https?:\/\/pdf\.printfriendly\.com\/pdfs\/make$/.test(destUrl) &&
    ...
    partial(xssReq) {
      ...
      if (/^https:\/\/www\.paypal\.com\/(?:[\w\-]+\/)?cgi-bin\/webscr\b/.test(destUrl)) {
        ...
        /^https?:\/\/apps\.facebook\.com\//.test(srcUrl) &&
        Legacy.getPref("filterXExceptions.fbconnect")
      ) {
        skipRx = /&invite_url=javascript[^&]+/ // Zynga stuff
      } else if (
        /^https?:\/\/l\.yimg\.com\/j\/static\/frame\?e=/.test(destUrl) &&

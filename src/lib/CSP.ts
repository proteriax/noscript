...
class CSP {
  ...
  buildBlocker(...types) {
    return this.build(
      ...types.map(type => `${type.name || type}-src ${type.value || "'none'"}`)
    )
  }
...
CSP.isEmbedType = type =>
  /\b(?:application|video|audio)\b/.test(type) && type !== "application/xhtml+xml"
...
CSP.patchDataURI = (uri, blocker) => {
  ...
  let patch =
    parts[0] +
    encodeURIComponent(`<meta http-equiv="${CSP.headerName}" content="${blocker}"/>`)
